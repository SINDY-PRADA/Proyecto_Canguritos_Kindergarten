...
<script>
  ...
  if (OK) {
	$.ajax({
	  url:"usuario?accion=editarUsuario&idUsuario="+id,
	...
	icon: "success",
			...
			if(OK){
				location.href="usuario?accion=editarUsuario&idUsuario="+id
			}
		  });
  } else {
...
<script>
	...
	if (OK) {
	$.ajax({
	url:"usuario?accion=eliminar&idUsuario="+id,
	success: function(res){
		console.log(res);
	}
	});
	...
	icon: "success",
			...
			if(OK){
				location.href="usuario?accion=eliminarUsuario&idUsuario="+id
			}
		});
	} else {
	swal("Se ha cancelado la operación", {
	...
	<section class="full-box dashboard-contentPage">
			...
			<div class="row">
				<div class="col-xs-12">
					...
					<div id="myTabContent" class="tab-content">
					  	<div class="tab-pane fade active in" id="usuario?accion=consultarUsuario">
							<div class="table-responsive">
								<table class="table table-hover text-center">
									...
									<tbody>
										<c:forEach var="usuarioList" items="${listar}">
										<tr>
										<td>${usuarioList.getIdUsuario()}</td>
										<td>${usuarioList.getUsername()}</td>
										<td>${usuarioList.getPass()}</td>
										<td>${usuarioList.getNivel()}</td>
										<td>
											<c:if test="${usuarioList.getEstadoUsuario() == true}">
												<span>Activo</span>
												</c:if>
												<c:if test="${usuarioList.getEstadoUsuario() == false}">
												<span>Inactivo</span>
												</c:if>
											</td>
										<td>
											<c:if test="${usuarioList.getEstadoUsuario() == false}">
												<a href="usuario?accion=estado&idUsuario=${usuarioList.getIdUsuario()}&estadoUsuario=true" class="activar" style="text-decoration:none"><span>Activar</span></a>
											
											</c:if>
												<c:if test="${usuarioList.getEstadoUsuario() == true}">
												<a href="usuario?accion=estado&idUsuario=${usuarioList.getIdUsuario()}&estadoUsuario=false" class="inactivar" style="text-decoration:none"><span>Inactivar</span></a>
											
											</c:if>
											 </td>
											 <td>
												<div>
													<!-- <a href="usuario?accion=editarUsuario&idUsuario=${usuarioList.getIdUsuario()}"><img src="assets/img/Editar.png" style="width: 25px;" onclick="return ConfirmDatos()"></a>  -->
													<a onclick="Editar([[${usuarioList.getIdUsuario()}]])">
                                                        <img src="assets/img/Editar.png" style="width: 25px;"></a>
															 
													<!-- <a href="usuario?accion=eliminarUsuario&idUsuario=${usuarioList.getIdUsuario()}"><img src="assets/img/eliminar.png" style="width: 25px;" onclick="return ConfirmEliminar()"></a>  -->
													<a onclick="Eliminar([[${usuarioList.getIdUsuario()}]])">
		...
		<!-- <script src="assets/js/validaciones/alertas.js"></script> -->
