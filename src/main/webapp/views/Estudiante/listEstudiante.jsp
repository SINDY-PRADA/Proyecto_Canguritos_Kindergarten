...
<script>
  ...
  if (OK) {
	$.ajax({
	  url:"estudiante?accion=editarEstudiante&idEstudiante="+id,
	...
	icon: "success",
			...
			if(OK){
				location.href="estudiante?accion=editarEstudiante&idEstudiante="+id
			}
		  });
  } else {
...
<script>
	...
	if (OK) {
	$.ajax({
	url:"estudiante?accion=eliminarEstudiante&idEstudiante="+id,
	success: function(res){
		console.log(res);
	}
	});
	...
	icon: "success",
			...
			if(OK){
				location.href="estudiante?accion=eliminarEstudiante&idEstudiante="+id
			}
		});
	} else {
	swal("Se ha cancelado la operación", {
							...
							<div class="table-responsive">
								<table class="table table-hover text-center">
									...
									<tbody>
										...
										<tr>
											<td>${estudianteList.getIdEstudiante()}</td>
											...
											<!-- <td>${estudianteList.getIdAcudiente()}</td> -->
										<td>
											<c:if test="${estudianteList.getEstadoEstudiante() == true}">
												<span>Activo</span>
												</c:if>
												<c:if test="${estudianteList.getEstadoEstudiante() == false}">
												<span>Inactivo</span>
												</c:if>
											</td>
										<td>
											<c:if test="${estudianteList.getEstadoEstudiante() == false}">
												<a href="estudiante?accion=estado&idEstudiante=${estudianteList.getIdEstudiante()}&estadoEstudiante=true" class="activar" style="text-decoration:none"><span>Activar</span></a>
												</c:if>
												<c:if test="${estudianteList.getEstadoEstudiante() == true}">
												<a href="estudiante?accion=estado&idEstudiante=${estudianteList.getIdEstudiante()}&estadoEstudiante=false"  class="inactivar" style="text-decoration:none"><span>Inactivar</span></a>
												</c:if>
											 </td>
											 <td>
												<div>
												   <!-- <a href="estudiante?accion=editarEstudiante&idEstudiante=${estudianteList.getIdEstudiante()}"><img src="assets/img/Editar.png" style="width: 25px;" onclick="return ConfirmDatos()"></a> -->
													<a onclick="Editar([[${estudianteList.getIdEstudiante()}]])">
                                                        <img src="assets/img/Editar.png" style="width: 25px;"></a>
												 
												 
												   <!-- <a href="estudiante?accion=eliminarEstudiante&idEstudiante=${estudianteList.getIdEstudiante()}"><img src="assets/img/eliminar.png" style="width: 25px;" onclick="return ConfirmEliminar()"></a> -->
												   <a onclick="Eliminar([[${estudianteList.getIdEstudiante()}]])">
